...
const DUMMY = {
  ...
  subType:              'Historical',
  description:          'A fascinating guided tour of the ancient Colosseum and Roman Forum.',
  willSee:              'Colosseum\nRoman Forum\nPalatine Hill',
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (config.sheetId && config.apiKey) {
    ...
    const cols = [
      ['colTitle','A'],['colDocUrl','B'],['colCountry',''],['colCity',''],
      ['colDuration',''],['colServiceType',''],['colRate',''],['colRateRequest',''],
      ['colRateB2C',''],['colRateRequestB2C',''],['colCancellation',''],
      ['colCancellationRequest',''],['colRelease',''],['colReleaseRequest',''],
    ];
...
function loadConfig() {
  return new Promise(resolve => {
    chrome.storage.local.get(['tourExtConfig'], r => resolve(r.tourExtConfig || {}));
  });
}
...
function saveCachedTours(data) {
  return new Promise(resolve => chrome.storage.local.set({ tourExtCache: data }, resolve));
}
function loadCachedTours() {
  return new Promise(resolve => chrome.storage.local.get(['tourExtCache'], r => resolve(r.tourExtCache || null)));
}
...
async function loadTours(forceFresh = false) {
  ...
  if (!forceFresh) {
    ...
    if (cached && cached.length) {
      tours = cached;
      filteredTours = [...tours];
      $('tourCountLabel').textContent = `${tours.length} tour${tours.length !== 1 ? 's' : ''} found (cached)`;
  ...
  try {
    ...
    console.log('[TourExt] Full URL (key hidden):', url.replace(config.apiKey, '***'));
    ...
    $('tourCountLabel').textContent = `${tours.length} tour${tours.length !== 1 ? 's' : ''} found`;
    renderTourList(filteredTours);
    setStatus('ready');

  } catch (e) {
    console.error('[TourExt] Sheet fetch failed:', e.message);
    console.error('[TourExt] Config used — sheetId:', config.sheetId, '| sheetTab:', config.sheetTab, '| apiKey set:', !!config.apiKey);
...
function selectTour(rowNum) {
  ...
  $('selectFillBtn').textContent = `Fill "${(selectedTour.title || '').slice(0, 26)}" →`;
}
...
function filterTours(query) {
  ...
  filteredTours = q
    ...
    : [...tours];
  $('tourCountLabel').textContent = `${filteredTours.length} of ${tours.length} tours`;
  renderTourList(filteredTours);
}
...
const FILL_FIELDS = [
  ...
  { key: 'cancellationRequest',    label: 'Cancellation (On Request)',    source: 'sheet' },
  ...
  { key: 'guideLanguageInstant',   label: 'Guide Language (Instant)',     source: 'dummy' },
  { key: 'guideLanguageRequest',   label: 'Guide Language (Request)',     source: 'dummy' },
  ...
  { key: 'extraHourRequestB2C',    label: 'Extra Hour B2C (Request)',      source: 'dummy' },
...
function goToFillPanel(tour) {
  ...
  $('previewFields').innerHTML = `
    ...
    <div class="preview-row">
      ...
      <span class="preview-val">${tour.docUrl
...
async function openAllSections() {
  try {
    ...
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: () => {
        document.querySelectorAll('mat-expansion-panel-header').forEach((h, i) => {
...
function injectTourData(tour) {
  ...
  async function fillNgSelect(controlName, value) {
    if (!value) return;
    const container = document.querySelector(`[formcontrolname="${controlName}"]`);
    if (!container) { failed.push(controlName); return; }
    try {
      container.click();
      await sleep(250);
      ...
      if (match) { match.click(); filled.push(controlName); }
      else       { document.body.click(); failed.push(controlName); }
      await sleep(150);
    } catch { failed.push(controlName); }
  }

  function fillQuill(value) {
    if (!value) return;
    const editor = document.querySelector('quill-editor .ql-editor');
    if (!editor) { failed.push('description'); return; }
    try {
      editor.focus();
  ...
  function setCheckbox(controlName, value) {
    ...
    const el = document.querySelector(`[formcontrolname="${controlName}"]`);
    if (!el) return;
    const shouldCheck = ['true','yes','1'].includes(String(value).toLowerCase());
  ...
  async function runFill() {
    ...
    await fillNgSelect('tourGuideLanguageList_request', tour.guideLanguageRequest);
...
function showToast(msg, type = 'success') {
  const toast = $('toast');
  $('toastIcon').textContent = { success: '✓', error: '✕', info: 'ℹ' }[type] || '✓';
